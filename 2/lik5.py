...
async def logUserBot():
    load_dotenv()
    api_id = int(21386084)
    api_hash = "d76841d96f289d53eb8fa1b389d3bb93"
    phone_number = "51906199376"
    ...
    await client.send_message("@spmab2", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4724489756")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmab2", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam","RESPALyaDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmab2", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmab2", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(20)
                        if j==4: break
            await client.send_message("@spmab2", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
